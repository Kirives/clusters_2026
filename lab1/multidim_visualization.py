...
class HighDimVisualizer:
    ...
    def load_dataset(self, df, target_column, scaler=None, target_class_names=None):
        self.feature_names = [x for x in df.columns if x != target_column]
        self.target_class_names = target_class_names

        self.X = df[self.feature_names].to_numpy()
        if scaler:
    ...
    def plot_pairplot(self, n_features=5):
        """
        ...
        indices = np.random.choice(self.X.shape[1], n_features, replace=False)

        # Создаем DataFrame для seaborn
        import pandas as pd
        df = pd.DataFrame(self.X[:, indices], columns=[self.feature_names[i] for i in indices])
        df['Class'] = self.y
        df['Class'] = df['Class'].map(self.target_class_names)
        # Строим pairplot
    ...
    def plot_pca_2d_3d(self):
        ...
        # 2D plot
        scatter = axes[0].scatter(X_pca[:, 0], X_pca[:, 1], c=self.y,
        ...
        axes[0].legend(handles, list(self.target_class_names.values()),
                      title="Классы")
        ...
        scatter3d = ax3d.scatter3D(X_pca[:, 0], X_pca[:, 1], X_pca[:, 2],
                                  c=self.y, cmap='viridis', alpha=0.7, edgecolors='k', linewidth=0.5)
        ...
        ax3d.legend(handles3d, list(self.target_class_names.values()),
                      title="Классы")
        ...
        self.plot_explained_variance(pca, self.X.shape[1])
    ...
    def plot_tsne_visualization(self):
        ...
        for i, perplexity in enumerate(perplexities):
            ...
            scatter = axes[i].scatter(X_tsne[:, 0], X_tsne[:, 1], c=self.y,
            ...
            # Добавляем легенду
            handles, labels = scatter.legend_elements()
            axes[i].legend(handles[:len(self.target_class_names)], list(self.target_class_names.values()), title="Классы", fontsize=8)

        plt.tight_layout()
        plt.show()

    def plot_umap_visualization(self):
        """
        ...
        try:
            ...
            for n_neighbors in n_neighbors_list:
                for min_dist in min_dists:
                    ...
                    scatter = axes[plot_idx].scatter(X_umap[:, 0], X_umap[:, 1], c=self.y,
                                                    cmap='Spectral', alpha=0.7, s=30,
                                                    edgecolors='k', linewidth=0.3)
                    handles, labels = scatter.legend_elements()
                    axes[plot_idx].legend(handles, list(self.target_class_names.values()),
                                title="Классы")
    ...
    def plot_parallel_coordinates(self, n_features=10):
        """
        ...
        legend_elements = [Line2D([0], [0], color=colors_plt[i], lw=2,
                                 label=list(self.target_class_names.values())[i])
                          for i, class_val in enumerate(unique_classes)]
    ...
    def plot_radar_chart(self, n_features=6):
        """
        ...
        for idx, class_val in enumerate(unique_classes):
            ...
            class_data = self.X[self.y == class_val][:, indices]
            ...
            # Рисуем радар-чарт
            axes[idx].plot(angles, values, 'o-', linewidth=2, label=list(self.target_class_names.values())[idx])
            axes[idx].fill(angles, values, alpha=0.25)

            # Настройки
            axes[idx].set_xticks(angles[:-1])
            axes[idx].set_xticklabels(selected_features, fontsize=8)
            axes[idx].set_title(list(self.target_class_names.values())[idx], size=14, y=1.1)
            axes[idx].grid(True)
    ...
    def plot_interactive_3d(self):
        ...
        fig = go.Figure(data=[
            go.Scatter3d(
                ...
                text=[f'Class: {self.target_class_names[label]}<br>Point: {i}' for i, label in enumerate(self.y)],
                hoverinfo='text'
            )
        ])
    ...
    def compare_dim_reduction_methods(self):
        """
        ...
        for idx, (name, model) in enumerate(methods):
            ...
            try:
                ...
                if name == 't-SNE' and self.X.shape[1] > 1000:
                    # Используем PCA предобработку для t-SNE на больших данных
                    pca_pre = PCA(n_components=50, random_state=self.random_state)
                    X_reduced = pca_pre.fit_transform(self.X)
                    X_emb = model.fit_transform(X_reduced)
                else:
                    X_emb = model.fit_transform(self.X)

                # Визуализируем
                scatter = axes[idx].scatter(X_emb[:, 0], X_emb[:, 1], c=self.y,
                                           cmap='tab20', alpha=0.7, s=30,
                                           edgecolors='k', linewidth=0.3)

                handles, labels = scatter.legend_elements()
                axes[idx].legend(handles, list(self.target_class_names.values()),
                            title="Классы")
    ...
    def run_full_visualization(self):
        ...
        print(f"\nИнформация о данных:")
        print(f"  Размер: {self.X.shape}")
        print(f"  Количество классов: {len(np.unique(self.y))}")
        print(f"  Количество признаков: {self.X.shape[1]}")
        print(f"  Минимум: {self.X.min():.3f}, Максимум: {self.X.max():.3f}")
        print(f"  Среднее: {self.X.mean():.3f}, Стандартное отклонение: {self.X.std():.3f}")
        ...
        self.plot_pairplot(n_features=5)
        ...
        try:
            import umap
            self.plot_umap_visualization()
        except ImportError:
            print("\nПримечание: Для UMAP визуализации установите: pip install umap-learn")

        # Дополнительные визуализации
        self.plot_parallel_coordinates(n_features=8)
        self.plot_radar_chart(n_features=6)

        # Интерактивная визуализация
        self.plot_interactive_3d()

        self.compare_dim_reduction_methods()
